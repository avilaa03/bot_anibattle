...
module.exports = async (client, interaction, inventoryCollect, inventoryEnd) => {
    ...
    const generateEmbed = (page) => {
        ...
        const lista = pageCards.map((card, i) => {
            ...
            return `\`${String(start + i + 1).padStart(2, '0')}\` ${meta.emoji} **${ui.cardName(card.name, t.locale)}**${isFav ? ' ⭐' : ''}\n`
                + `└ ${card.series || t('comum.traco')} • ${t('atributos.ovr')} **${getOvr(card)}** • ${ui.coins(card.marketValue || 0, t.locale)}`;
        }).join('\n');
