...
async function fichaRun(client, interaction) {
    ...
    if (encontradas.length === 0) {
        const descricao = numeroBuscado != null
            ...
            : t('desejar.sem_nome', { nome: nomeBuscado });
    ...
    const montarFicha = async (i) => {
        ...
        const render = await renderCard(carta, { moldura, locale: t.locale });
        ...
        const embed = ui.base(meta.color)
            ...
            .setTitle(`${formatarNumero(carta.numero, totalCatalogo)} • ${meta.emoji} ${ui.cardName(carta.name, t.locale)}`)
            .setDescription([
            ...
            .addFields(
                {
                    ...
                    value: descobertaEm ? `<t:${Math.floor(new Date(descobertaEm).getTime() / 1000)}:D>` : t('comum.traco'),
                    inline: true
                },
                {
                ...
                },
                {
                    ...
                    value: ui.coins((carta.overall ?? 0) * 10, t.locale),
                    inline: true
                },
                {
                    ...
                    value: t('desejar.n_jogadores', { n: await wishlist.contarDesejos(carta._id) }),
                    inline: true
                }
            )
            .setImage(render.url)
